...
//    Sharp Peak Kernel
    real_t scale = reference_12 / (std::pow(radius, 4) * EIGEN_PI);
...
real_t CalculateDensity(
    ...
    for (const auto &position: positions) {
        ...
        density += Constants::particleMass * influence;
    }
...
vector2 CalculateDensityGradient(
    ...
    // Currently doesn't do much due to every particle having identical Constants::particleMass. May be updated in the future.
    ...
    for (int i{0}; i < Constants::particleNumber; i++) {
        ...
        propertyGradient += -dir * slope * Constants::particleMass;
    }
...
real_t CalculateProperty(
    ...
    for (int i{0}; i < Constants::particleNumber; i++) {
        ...
        property += particleProperty[i] * influence * Constants::particleMass / density;
    }
...
vector2 CalculatePropertyGradient(
    ...
    for (int i{0}; i < Constants::particleNumber; i++) {
        ...
        propertyGradient += -particleProperty[i] * dir * slope * Constants::particleMass / density;
    }
...
vector2 CalculatePressureForce(
    ...
    for (int i{0}; i < Constants::particleNumber; i++) {
        ...
        real_t slope = SmoothingKernelDerivative(Constants::smoothingRadius, dst);
        real_t density = densities[i];
        propertyGradient += -ConvertDensityToPressure(density) * dir * slope * Constants::particleMass / density;
    }
